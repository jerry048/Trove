 ...
 *   include/net/tcp.h and passes BBRX_TCP_CA_* feature macros.
 */
...
static inline u32 bbrx_tcp_snd_cwnd(const struct tcp_sock *tp)
{
	return tp->snd_cwnd;
}

static inline void bbrx_tcp_snd_cwnd_set(struct tcp_sock *tp, u32 val)
{
...
#define tcp_snd_cwnd(tp) bbrx_tcp_snd_cwnd(tp)
#define tcp_snd_cwnd_set(tp, val) bbrx_tcp_snd_cwnd_set(tp, val)
#endif

static inline u32 bbrx_random_u32_below(u32 ceil)
{
...
/* After 10 rounds without that growth, BBRx estimates the pipe is full. */
...
#if defined(BBRX_TCP_CA_HAS_TSO_SEGS)
#if defined(BBRX_TCP_CA_TSO_SEGS_1_ARG)
...
#if (defined(BBRX_TCP_CA_HAS_CWND_EVENT) && defined(BBRX_TCP_HAS_CA_EVENT_TX_START)) || 	!defined(BBRX_TCP_CA_PROBED)
...
#if defined(BBRX_TCP_CA_HAS_CWND_EVENT_TX_START) || 	(!defined(BBRX_TCP_CA_PROBED) && 	 LINUX_VERSION_CODE >= KERNEL_VERSION(7, 1, 0))
...
static void bbr_reset_probe_bw_mode(struct sock *sk)
{
	...
	bbr->cycle_idx = CYCLE_LEN - 1 - bbrx_random_u32_below(bbr_cycle_rand);
 ...
 * The Makefile probes include/net/tcp.h and passes BBRX_TCP_CA_* feature macros.
 */
#if defined(BBRX_TCP_CA_CONG_CONTROL_4_ARGS) || 	(!defined(BBRX_TCP_CA_PROBED) && 	 LINUX_VERSION_CODE >= KERNEL_VERSION(6, 10, 0))
...
static struct tcp_congestion_ops tcp_bbrx_cong_ops __read_mostly = {
...
#elif defined(BBRX_TCP_CA_HAS_MIN_TSO_SEGS) || !defined(BBRX_TCP_CA_PROBED)
...
static int __init bbrx_register(void)
{
	BUILD_BUG_ON(sizeof(struct bbr) > ICSK_CA_PRIV_SIZE);
	return tcp_register_congestion_control(&tcp_bbrx_cong_ops);
}

static void __exit bbrx_unregister(void)
{
	tcp_unregister_congestion_control(&tcp_bbrx_cong_ops);
}

module_init(bbrx_register);
module_exit(bbrx_unregister);
...
MODULE_DESCRIPTION("TCP BBRx (BBR variant for DKMS)");